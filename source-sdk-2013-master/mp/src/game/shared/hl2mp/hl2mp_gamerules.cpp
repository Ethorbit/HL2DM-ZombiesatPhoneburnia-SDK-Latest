...
bool CHL2MPRules::ShouldCollide(int collisionGroup0, int collisionGroup1)
{
	...
	if ((collisionGroup0 == COLLISION_GROUP_DEBRIS_TRIGGER || collisionGroup0 == COLLISION_GROUP_PLAYER_MOVEMENT) &&
		collisionGroup1 == COLLISION_GROUP_WEAPON)
	{
		return false;
	}

	// Disabled player collisions! BY: ETHORBIT.
	// Many errors have occured at this zone. Mostly fixed. Experiencing collision problems? Remove the disabled player collision code.
	// First 5 lines fix - NPC COLLISION Including all npcs, the second line focuses on npcs that aren't included with collision_group_npc.
	if ((collisionGroup1 == COLLISION_GROUP_NPC || collisionGroup1 == COLLISION_GROUP_PROJECTILE || collisionGroup1 == HL2COLLISION_GROUP_HEADCRAB ||
		collisionGroup1 == HL2COLLISION_GROUP_PLASMANODE || collisionGroup1 == HL2COLLISION_GROUP_ANTLION || collisionGroup1 == COLLISION_GROUP_NONE ||
		collisionGroup1 == HL2COLLISION_GROUP_HOUNDEYE || collisionGroup1 == COLLISION_GROUP_PUSHAWAY) &&
		collisionGroup0 == COLLISION_GROUP_DEBRIS_TRIGGER || collisionGroup0 == COLLISION_GROUP_PLAYER_MOVEMENT)
	{
		return true;
	}

	// Allows players to go through each other, and everything else below is my struggle to keep all other collisions the same.
	if ((collisionGroup0 == COLLISION_GROUP_DEBRIS_TRIGGER || collisionGroup0 == COLLISION_GROUP_PLAYER_MOVEMENT) &&
		collisionGroup0 == COLLISION_GROUP_DEBRIS_TRIGGER || collisionGroup0 == COLLISION_GROUP_PLAYER_MOVEMENT)
	{
		return false;
	...
	// EXTREMELY IMPORTANT so I had to define it more than once for player collision script to be compatible with NPCS.
	if (collisionGroup1 == COLLISION_GROUP_NPC && collisionGroup0 == COLLISION_GROUP_PLAYER)
	{
		return true;
		if ((collisionGroup0 == COLLISION_GROUP_DEBRIS_TRIGGER || collisionGroup0 == COLLISION_GROUP_PLAYER_MOVEMENT) &&
			collisionGroup0 == COLLISION_GROUP_DEBRIS_TRIGGER || collisionGroup0 == COLLISION_GROUP_PLAYER_MOVEMENT)
	...
	// REQUIRED OR ELSE PLAYER COLLISION SCRIPT EFFECTS PROP BUMP SPEED!
	if ((collisionGroup0 == COLLISION_GROUP_DEBRIS_TRIGGER || collisionGroup0 == COLLISION_GROUP_PLAYER) &&
		collisionGroup0 == COLLISION_GROUP_DEBRIS || collisionGroup0 == COLLISION_GROUP_DEBRIS)
	{
		return false;
	}
	if ((collisionGroup0 == COLLISION_GROUP_PLAYER_MOVEMENT || collisionGroup0 == COLLISION_GROUP_PLAYER_MOVEMENT) &&
		collisionGroup0 == COLLISION_GROUP_NONE || collisionGroup0 == COLLISION_GROUP_NONE)
	{
		return true;
	}
	// PATCHES HEADCRABS BEING MAGICALLY FORCED BACK WHEN ATTEMPTING TO HIT PLAYER. Player collision problem.
	// This may have been solved at the very top, but it's good to take extreme measures by over-defining this.
	if (collisionGroup0 == HL2COLLISION_GROUP_HEADCRAB && (collisionGroup1 == COLLISION_GROUP_DEBRIS_TRIGGER || COLLISION_GROUP_PLAYER_MOVEMENT))
	{
		return true;
	}
	if (collisionGroup1 == HL2COLLISION_GROUP_HEADCRAB && (collisionGroup0 == COLLISION_GROUP_DEBRIS_TRIGGER || COLLISION_GROUP_PLAYER_MOVEMENT))
	{
		return true;
	}
	if (collisionGroup0 == HL2COLLISION_GROUP_HEADCRAB && (collisionGroup1 == COLLISION_GROUP_INTERACTIVE))
	{
		return true;
	}
	if (collisionGroup1 == HL2COLLISION_GROUP_HEADCRAB && (collisionGroup0 == COLLISION_GROUP_INTERACTIVE))
	{
		return true;
	}
...
void CHL2MPRules::InitDefaultAIRelationships( void )
{
	...
	*/
	...
	CBaseCombatCharacter::SetDefaultRelationship(CLASS_ZOMBIE,			CLASS_ANTLION,			D_NU, 0);
	...
	CBaseCombatCharacter::SetDefaultRelationship(CLASS_ZOMBIE,			CLASS_VORTIGAUNT,		D_NU, 0);		

...
//	void WeaponIdle( void );
	...
	virtual float SetDamage(void) { return 500.0; };
	void DryFire( void );
	virtual float GetFireRate(void) { return 0.7; };
#define SHOTGUNDAMAGE 500.0f
...
void CWeaponShotgun::PrimaryAttack( void )
{
	...
	this->SetDamage();
	SetDamage();
	// Shotgun damage modifed by: ETHORBIT.
	if (&CBaseCombatWeapon::PrimaryAttack)
	{
		SetDamage();
	}
	...
	pPlayer->SetDamage(500);
	
	QAngle punch;
	punch.Init( SharedRandomFloat( "shotgunpax", -2, -1 ), SharedRandomFloat( "shotgunpay", -2, 2 ), 0 );
	pPlayer->ViewPunch( punch );
	info.m_flDamage = SHOTGUNDAMAGE;
	info.m_flDamageForceScale = SHOTGUNDAMAGE;
...
void CWeaponShotgun::SecondaryAttack( void )
{
	...
	SetDamage();
	// Shotgun damage modifed by: ETHORBIT.
	if (&CBaseCombatWeapon::SecondaryAttack)
	{
		SetDamage();
	}

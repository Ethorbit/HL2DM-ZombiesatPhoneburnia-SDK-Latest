...
#define crowbar 2.1f
#define stunstick 1.7f
#define stunstickcount 116.7f
#define crossbowcount 200
#define crossbowcount2 500
#define crossbow 250.0f
#define shotgundamage 1.3f
#define bulletdamage 2.0f
#define blast 2.0f
#define crush 2.0f
...
//Default fix by: Ethorbit.
ConVar	sk_zombie_health( "sk_zombie_health","250");
...
{
	// Crowbar and Stunstick damage patch required since I created the zombie body and head dmg patch By: ETHORBIT
	CTakeDamageInfo inputInfo = info;

	if (inputInfo.GetDamageType() & DMG_CLUB)
	{
		inputInfo.ScaleDamage(crowbar);
	}

	// Workaround to detect the amount of damage by stunstick since stunstick and crowbar are both DMG_CLUB By: ETHORBIT
	if (inputInfo.GetDamageType() & DMG_CLUB)
	{
		if (inputInfo.GetMaxDamage() == stunstickcount)
		{
			inputInfo.ScaleDamage(stunstick);
		}
	}

	// Crossbow got messed up for some odd reason so I have to scale this damage too, even though it deals as much damage as zombie hp By: ETHORBIT
	// Using workaround method to prevent future unwanted bugs.
	if (inputInfo.GetDamageType() & DMG_BULLET | DMG_NEVERGIB)
	{
		if (inputInfo.GetMaxDamage() == crossbowcount)
		{
			inputInfo.ScaleDamage(crossbow);
		}
	}

	//And because it's stubborn I just filled it with a bunch of possibilities so it doesn't end up magically fucking up.
		if (inputInfo.GetMaxDamage() == crossbowcount)
		{
			inputInfo.ScaleDamage(crossbow);
		}

		if (inputInfo.GetMaxDamage() == crossbowcount2)
		{
			inputInfo.ScaleDamage(crossbow);
		}

	// Explosive damage ALSO got messed up.. By: ETHORBIT
	if (inputInfo.GetDamageType() & DMG_BLAST)
		{
			inputInfo.ScaleDamage(blast);
		}

	// And so did props.... By: ETHORBIT
	if (inputInfo.GetDamageType() & DMG_CRUSH)
	{
		inputInfo.ScaleDamage(crush);
	}

	// Oh yeah, I forgot WE NEED TO PATCH THE SHOTGUN DAMAGE TOO!! By: ETHORBIT
	if (inputInfo.GetDamageType() & DMG_BUCKSHOT)
	{
		inputInfo.ScaleDamage(shotgundamage);
	}

	// There are easier ways at this point but I'm just sticking with this weird method. :/ By: ETHORBIT
	if (inputInfo.GetDamageType() & DMG_BULLET)
	{
		inputInfo.ScaleDamage(bulletdamage);
	}

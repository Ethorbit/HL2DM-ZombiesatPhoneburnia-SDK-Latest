...
#define ZOMBIE_BULLET_DAMAGE_SCALE_HEAD 0.3
#define SHOTGUNDAMAGE 0.9
#define crush
...
// Damages modified by: Ethorbit.
ConVar	sk_zombie_dmg_one_slash( "sk_zombie_dmg_one_slash","4");
ConVar	sk_zombie_dmg_both_slash( "sk_zombie_dmg_both_slash","8");
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
// SHOTGUN DAMAGE PATCH BY: ETHORBIT
...
void CNPC_BaseZombie::TraceAttack( const CTakeDamageInfo &info, const Vector &vecDir, trace_t *ptr, CDmgAccumulator *pAccumulator )
{
	...
	if( infoCopy.GetDamageType() & DMG_BUCKSHOT )
	{
		...
		infoCopy.ScaleDamage( 2.4 );
	}


	if (GetDamageType() & DMG_BULLET)
	{
		// Zombie gets across-the-board damage reduction for buckshot. This compensates for the recent changes which
		// make the shotgun much more powerful, and returns the zombies to a level that has been playtested extensively.(sjb)
		// This normalizes the buckshot damage to what it used to be on normal (5 dmg per pellet. Now it's 8 dmg per pellet). 
		infoCopy.ScaleDamage(115.0);
	}
...
int CNPC_BaseZombie::OnTakeDamage_Alive( const CTakeDamageInfo &inputInfo )
{
	...
	// Punt damage fix By: ETHORBIT
	if (inputInfo.GetDamageType() & DMG_CRUSH)
	{
		info.ScaleDamage(3);
	}

	// Blast damage fix By: ETHORBIT
	if (inputInfo.GetDamageType() & DMG_BLAST)
	{
		info.ScaleDamage(2);
	}
	// ZOMBIE HEAD AND BODY DAMAGE SIMILAR. FIX BY: ETHORBIT.
	// Zombies used to die instantly in the head, now body or head takes 1 smg1 clip.
	...
	{
		info.ScaleDamage(ZOMBIE_BULLET_DAMAGE_SCALE_HEAD);
	}

	if (!m_bHeadShot && (info.GetDamageType() & DMG_BUCKSHOT))
	{
		info.ScaleDamage(SHOTGUNDAMAGE);
	}

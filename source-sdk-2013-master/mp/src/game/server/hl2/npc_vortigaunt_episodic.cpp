...
#define	VORTIGAUNT_HEAL_RECHARGE			999999.0		// How long to rest between heals
#define	VORTIGAUNT_ZAP_GLOWGROW_TIME		0.5			// How long does glow last
#define	VORTIGAUNT_HEAL_GLOWGROW_TIME		0.0			// How long does glow last
...
#define	PLAYER_CRITICAL_HEALTH_PERC			0.0f

#define blast 2.0f
// Edited to NOT HEALING. By: Ethorbit.
...
ConVar sk_vortigaunt_health("sk_vortigaunt_health", "350");
ConVar sk_vortigaunt_dmg_claw("sk_vortigaunt_dmg_claw", "25");
ConVar sk_vortigaunt_dmg_rake("sk_vortigaunt_dmg_rake", "50");
ConVar sk_vortigaunt_dmg_zap("sk_vortigaunt_dmg_zap", "500");
ConVar sk_vortigaunt_zap_range("sk_vortigaunt_zap_range", "9999", FCVAR_NONE, "Range of vortigaunt's ranged attack (feet)");
...
// Interactions
//-----------------------------------------------------------------------------
int	g_interactionVortigauntStomp = 1;
int	g_interactionVortigauntStompFail = 1;
int	g_interactionVortigauntStompHit = 1;
int	g_interactionVortigauntKick = 0;
int	g_interactionVortigauntClaw = 1;
...
void CNPC_Vortigaunt::TraceAttack(const CTakeDamageInfo &inputInfo, const Vector &vecDir, trace_t *ptr, CDmgAccumulator *pAccumulator)
{
	...
	case HITGROUP_LEFTLEG:
		// Removed | BLAST so I can set the damage. By: ETHORBIT.
	...
	case 10:
		...
		{
			info.ScaleDamage(blast);
		}

		// always a head shot
		ptr->hitgroup = HITGROUP_LEFTLEG;
		break;
	}
...
{
	return false;
	// Heal behavior is never available. By: ETHORBIT.
}
...
{
	return false;
	// I should NEVER heal targets. By: Ethorbit
}
...
int CNPC_Vortigaunt::SelectSchedule(void)
{
	...
	{
		m_flNextHealTime = 99999;
		// WRONG! Never recharge. By: Ethorbit.
	...
	if (HasCondition(COND_VORTIGAUNT_CAN_HEAL))
		return false; 
	// Wrong! Don't heal a player if they can be healed! By: Ethorbit.
...
void CNPC_Vortigaunt::PrescheduleThink(void)
{
	...
	//Don't do any heal schedules! By: Ethorbit.
...
void CNPC_Vortigaunt::ArmBeam(int beamType, int nHand)
{
	...
	AngleVectors(GetLocalAngles(), &forward, &right, &down);
	Vector vecSrc = GetLocalOrigin() + down * 36 + right * side * 16 + forward * 32;

	for (int i = 0; i < 3; i++)
	{
		Vector vecAim = forward * random->RandomFloat(-1, 1) + right * side * random->RandomFloat(0, 1) + down * random->RandomFloat(-1, 1);
		trace_t tr1;
...
void CNPC_Vortigaunt::ZapBeam(int nHand)
{
	...
	if (g_debug_vortigaunt_aim.GetBool())
	{
		NDebugOverlay::Line(tr.startpos, tr.endpos, 255, 0, 0, true, 5.0f);
	}
	...
	if (pEntity != NULL && m_takedamage)
	{
		if (g_debug_vortigaunt_aim.GetBool())
		{
			NDebugOverlay::Box(tr.endpos, -Vector(20, 20, 20), Vector(20, 20, 20), 255, 0, 0, 8, 5.0f);
		}
...
"		COND_VORTIGAUNT_DISPEL_ANTLIONS"
// "		COND_VORTIGAUNT_CAN_HEAL"
// Do not get interrupted for the need to heal! By: Ethorbit
);
...
void CVortigauntChargeToken::SeekThink(void)
{
	...
	vecOffset += vecUp * cos(gpGlobals->curtime * 10.0f) * -100.0f * gpGlobals->frametime;
	vecOffset += vecRight * sin(gpGlobals->curtime * 15.0f) * 100.0f * gpGlobals->frametime;

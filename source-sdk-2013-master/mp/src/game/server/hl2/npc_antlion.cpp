...
// A LOT OF CHANGES WERE MADE HERE.
// 1. Farther jumps
// 2. Wider view angle
// 3. More health
// 4. More frequent jumps
// 5. Moves more
// Changes made by ETHORBIT.
//=============================================================================//
...
// base antlion stuff
// Health and damages modified by: Ethorbit.
ConVar	sk_antlion_health( "sk_antlion_health", "200" );
ConVar	sk_antlion_swipe_damage( "sk_antlion_swipe_damage", "4" );
ConVar	sk_antlion_jump_damage( "sk_antlion_jump_damage", "4" );
ConVar  sk_antlion_air_attack_dmg( "sk_antlion_air_attack_dmg", "4" );
...
#define	ANTLION_JUMP_MIN			50.0f

#define	ANTLION_JUMP_MAX_RISE		1512.0f
#define	ANTLION_JUMP_MAX			3024.0f

#define	ANTLION_MIN_BUGBAIT_GOAL_TARGET_RADIUS	1512
...
#define	ANTLION_BUGBAIT_NAV_TOLERANCE	400

#define	ANTLION_OBEY_FOLLOW_TIME	10.0f

#define crowbar 0.5f
#define stunstick 1.3f
#define stunstickcount 116.7f
//==================================================
// AntlionSquadSlots
//==================================================
...
CNPC_Antlion::CNPC_Antlion( void )
{
	...
	m_flAlertRadius	= 512.0f;
	m_flFieldOfView	= 1.5f;
...
#define	ANTLION_VIEW_FIELD_NARROW	0.25f
...
#define NUM_CHASE_POSITION_ATTEMPTS		6
...
bool CNPC_Antlion::GetPathToSoundFleePoint( int soundType )
{
	...
	if ( pHint != NULL )
	{
		...
		AI_NavGoal_t goal( vecFleeGoal, (Activity) ACT_ANTLION_RUN_AGITATED, 256, AIN_DEF_FLAGS );
	...
	// Find a route to that position
	AI_NavGoal_t goal( vecFleeGoal + Vector( 0, 0, 8 ), (Activity) ACT_ANTLION_RUN_AGITATED, 1512, AIN_DEF_FLAGS );
...
{
	const float MAX_JUMP_RISE		= 1512;
	const float MAX_JUMP_DROP		= 1512;
	const float MAX_JUMP_DISTANCE	= 3024;
	const float MIN_JUMP_DISTANCE   = 50;
...
int CNPC_Antlion::OnTakeDamage_Alive( const CTakeDamageInfo &info )
{
	...
	// Crowbar and Stunstick damage patch required since zombie body and head dmg patch By: ETHORBIT

	if (newInfo.GetDamageType() & DMG_CLUB )
	{
		newInfo.ScaleDamage(crowbar);
	}

	// Workaround to detect the amount of damage by stunstick since stunstick and crowbar are both DMG_CLUB By: ETHORBIT
	if (newInfo.GetDamageType() & DMG_CLUB)
	{
		if (info.GetMaxDamage() == stunstickcount)
		{
			newInfo.ScaleDamage(stunstick);
		}
	}
...
void CNPC_Antlion::GatherEnemyConditions( CBaseEntity *pEnemy )
{
	...
	if ( HasCondition( COND_ENEMY_UNREACHABLE ) == false )
	{
		...
		if ( predLength > m_flEludeDistance )
		{
			...
			if ( ( predSpeed > 1512.0f ) && ( DotProduct( predVelDir, predDir ) > 0.0f ) )
			{
...
bool CNPC_Antlion::OverrideMoveFacing( const AILocalMoveGoal_t &move, float flInterval )
{
	...
	if ( IsCurSchedule( SCHED_ANTLION_FLEE_THUMPER ) )
	{
	...
	else if ( GetEnemy() && GetNavigator()->GetMovementActivity() == ACT_RUN )
  	{
		...
		if ( UTIL_DistApprox( vecEnemyLKP, GetAbsOrigin() ) < 1512 )
		{
...
AI_BEGIN_CUSTOM_NPC( npc_antlion, CNPC_Antlion )
	...
	(
		...
		"		TASK_ANTLION_FIND_BURROW_IN_POINT	1512"
		"		TASK_RUN_PATH						0"
	...
	(
		...
		"	Tasks"
		"		TASK_SET_TOLERANCE_DISTANCE		228"
